...
alpha=${2:-1e-1}
eta_lbd=${3:-5}
task=${4:-gsm8k}  # Default task, can be overridden
...
# For the typical HF cache layout ".../models--<org>--<model>/snapshots/<hash>", grab the directory between
...
# and strip an optional "models--" prefix, as well as any prefix ending with "--" to keep only the final segment
...
alignment_base=${path_after_slash}_sft_alignment_sft_finetune_seed${seed}_job${job_id}
...
echo "Downstream Task: $task"
echo "Poison ratios to iterate over: ${poison_ratios[@]}"
...
# --------------------------------------------------------
# The script is located in script/poison_ratio, so move two levels up to reach project root.
cd ../../ || { echo "Failed to cd to project root"; exit 1; }
...
cd poison/evaluation || { echo "Failed to cd to poison/evaluation"; exit 1; }
...
for poison_ratio in "${poison_ratios[@]}"; do
    ...
    # Unique identifier per poison ratio run
    base_name=${path_after_slash}_sft_alignment_sft_seed${seed}_job${job_id}_pr${poison_ratio}
    ...
    # ------------- Faithfulness / downstream eval -------------
    cd ${task} || { echo "Failed to cd to ${task}"; exit 1; }
