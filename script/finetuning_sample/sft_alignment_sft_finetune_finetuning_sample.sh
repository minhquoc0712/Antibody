...
seed=${1:-5}
task=${2:-gsm8k}  # Default task, can be overridden
...
# and strip an optional "models--" prefix, and trim any prefix ending with "--" to keep only the final model name segment
...
# Removed printing of eta_lbd and alpha
...
for finetuning_sample_num in "${finetuning_samples[@]}"; do
	...
	# Common suffix for all paths in this configuration (include method to avoid collisions)
	base_name=${path_after_slash}_sft_alignment_sft_finetune_seed${seed}_job${job_id}_fs${finetuning_sample_num}_pr${poison_ratio_fixed}
