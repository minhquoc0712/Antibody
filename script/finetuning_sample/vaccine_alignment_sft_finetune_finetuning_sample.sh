...
# and strip an optional "models--" prefix. Also trim any prefix ending with "--" to keep only the final model name segment.
...
alignment_base=${path_after_slash}_vaccine_sft_seed${seed}_job${job_id}
...
for finetuning_sample_num in "${finetuning_samples[@]}"; do
	...
	base_name=${path_after_slash}_seed${seed}_job${job_id}_fs${finetuning_sample_num}_pr${poison_ratio_fixed}
