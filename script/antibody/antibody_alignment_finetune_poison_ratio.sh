...
for poison_ratio in "${poison_ratios[@]}"; do
    ...
    # Simplified path suffix for downstream tasks (only model id + job id + poison ratio)
    base_name=${path_after_slash}_antibody_job${job_id}_pr${poison_ratio}
    ...
    lora_folder=${alignment_ckpt}
    wandb_run_name=${task}_${base_name}_antibody_job${job_id}_seed${seed}_pr${poison_ratio}
