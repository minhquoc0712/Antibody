...
alignment_base=${path_after_slash}_antibody_job${job_id}
alignment_ckpt=ckpt/${alignment_base}
alignment_poison=data/poison/${alignment_base}

export WANDB_MODE="online"

echo "Antibody alignment + Weighted SFT with varying finetuning samples"
echo "Alignment Parameters:"
echo "  eta_lbd: $eta_lbd"
echo "  alpha: $alpha"
echo "  ema_momentum: $ema_momentum"
echo "  lambda2: $lambda2"
echo "  bad_sample_num: $bad_sample_num"
echo "  align_sample_num: $align_sample_num"
echo "Downstream Finetuning Parameters:"
echo "  task: $task"
echo "  fixed poison ratio: ${poison_ratio_fixed}"
echo "  finetuning_sample_num values: ${finetuning_samples[@]}"
echo "  tau: ${tau}"
echo "Environment:"
echo "  model path: $model_path"
echo "  short model id: $path_after_slash"
echo "  job id: $job_id"
echo "  seed: $seed"

cd ../../

echo "Starting Antibody alignment training..."
...
for finetuning_sample_num in "${finetuning_samples[@]}"; do
	echo "Starting ${task^^} training with finetuning_sample_num: $finetuning_sample_num and poison ratio: ${poison_ratio_fixed}..."

	benign_dataset=data/${task}.json
	# Common suffix for all paths in this configuration
	base_name=${path_after_slash}_antibody_seed${seed}_job${job_id}_fs${finetuning_sample_num}
