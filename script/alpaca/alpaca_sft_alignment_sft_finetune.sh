...
finetuning_epoch=100
finetuning_batch_size=16
finetuning_sample_num=700
...
alignment_base=${path_after_slash}_sft_sft_seed${seed}_job${job_id}
...
for task in alpaca; do
    echo "Starting ${task} training..."

    benign_dataset=data/${task}.json
    # Common suffix for all paths in this task
    base_name=${path_after_slash}_sft_alignment_sft_finetune_seed${seed}_job${job_id}
    ...
    CUDA_VISIBLE_DEVICES=0 python pred_eval.py \
        --lora_folder ../${output_dir} \
        --model_folder ${model_path} \
        --output_path ../data/${task}/${base_name}.json \
        --generator_name ${alignment_base}

    # echo "Currently using environment: $CONDA_DEFAULT_ENV"
    # conda deactivate
    ...
    # JUDGE_MODEL="alpaca_eval_gpt4"
    # echo "IS_ALPACA_EVAL_2 is: $IS_ALPACA_EVAL_2"
    # echo "JUDGE_MODEL is: $JUDGE_MODEL"
    # alpaca_eval --model_outputs ../data/${task}/${base_name}.json \
    #     --annotators_config ${JUDGE_MODEL} \
    #     --output_path ../data/${task}/${JUDGE_MODEL}_${base_name}
    # cd ..
done
