...
class BoosterAlignmentTrainer(Trainer):
    ...
    def init(self, harmful_datast):
        ...
        logger.debug(f"System evaluate mode: {getattr(self.args, 'system_evaluate', 'False')}")
        # Check if wandb is initialized
        if (getattr(self.args, "system_evaluate", "False") != "True") and wandb.run is None and hasattr(self.args, "eta_lbd"):
            wandb.init(
                project="Booster",
                name=f"booster_al{self.args.alpha}_etalb{self.args.eta_lbd}_k{self.args.num_ascent_steps}",
                config={
    ...
    def _log_metrics(self, metrics_dict):
        ...
        """
        if getattr(self.args, "system_evaluate", "False") == "True":
            return
        ...
        def step():
            ...
            for name, param in model.named_parameters():
                if param.requires_grad:
                    param.grad.data = (
                        safe_grads[name]
                        + self.args.eta_lbd * (harmful_grads[name] - perturb_grads[name])
                    )

            self.steps += 1

            # Compute and log metrics only every 10 steps (disabled in system evaluate mode)
            if (getattr(self.args, "system_evaluate", "False") != "True") and (self.steps % 10 == 0):
                diff_grads = {
                    name: harmful_grads[name] - perturb_grads[name]
                    for name in harmful_grads
                    if name in perturb_grads
                }
                final_grads = {
                    name: param.grad.data.clone()
                    for name, param in model.named_parameters()
            ...
            final_loss = alignment_safe_loss + self.args.eta_lbd * (
                harmful_loss - perturbed_loss
            )
